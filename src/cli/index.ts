...
import type { ScreenerCriteria } from "../types/index.js";

const program = new Command();

program
  .name("stockpulse")
  .description("AI-powered Indian stock research CLI")
  .version("1.0.0");
...
program
  ...
  .action(async (symbol: string) => {
    ...
    try {
      ...
      console.log(`  Open: ₹${quote.open.toFixed(2)}  High: ₹${quote.high.toFixed(2)}  Low: ₹${quote.low.toFixed(2)}`);
...
program
  ...
  .action(async (symbol: string, options: { strategy: string; capital: string; range: string }) => {
    ...
    try {
      ...
      console.log(`${chalk.dim("Final capital:")}    ₹${Math.round(result.finalCapital).toLocaleString()}`);
      console.log(`${chalk.dim("Total return:")}     ${result.totalReturn >= 0 ? chalk.green("+") : chalk.red("")}${result.totalReturn.toFixed(2)}%`);
      ...
      console.log(`${chalk.dim("Win rate:")}         ${chalk.cyan(result.winRate.toFixed(1) + "%")}`);
    } catch (error) {
      console.error(chalk.red(`Error: ${(error as Error).message}`));
      process.exit(1);
    }
  ...
  .action(async (options) => {
    ...
    if (options.add) {
      ...
      const answers = await inquirer.prompt([
        { type: "input", name: "symbol", message: "Symbol:", validate: (v: string) => v.length > 0 },
      ...
      if (entries.length === 0) {
        ...
        entries.forEach((e) => {
          ...
          console.log(`${color(e.date.split("T")[0])}  ${chalk.bold(e.symbol)}  ${color(e.action)}  ${e.quantity} @ ₹${e.price}`);
...
program
  ...
  .action(async (symbol: string, options: { model?: string }) => {
    ...
    const fundamentals = { symbol: symbol.toUpperCase(), note: "Fetching live fundamentals from Screener.in" };
...
program
  ...
  .action((options) => {
    ...
    console.log(chalk.dim("2. After authorizing, you'll be redirected. Copy the `auth_code` value."));
    if (options.code) {
      client.authenticate(options.code).then(() => {
